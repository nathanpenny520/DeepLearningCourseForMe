...
打开 `01-tokenization/01-tokenization.ipynb`：
...
2. 仍没有则手动指定：`Python Environments...` → `Enter interpreter path...` → 浏览到 `llm/venv/bin/python`（Mac）或 `llm\venv\Scripts\python.exe`（Windows）
...
在网页中 **Kernel → Change Kernel** 选择 `Python (llm)`。
