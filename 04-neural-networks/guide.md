...
> 以下命令均在**仓库根目录**下开始执行，先进入模块目录 `cd 04-neural-networks`。
...
打开 `01-mlp-from-scratch/01-mlp-from-scratch.ipynb`：
...
2. 仍没有则手动指定：`Python Environments...` → `Enter interpreter path...` → 浏览到 `04-neural-networks/venv/bin/python`（Mac）或 `04-neural-networks\venv\Scripts\python.exe`（Windows）
...
在网页中 **Kernel → Change Kernel** 选择 `Python (neural-networks)`。
...
jupyter kernelspec remove neural-networks
```
