...
2. 仍没有则手动指定：`Python Environments...` → `Enter interpreter path...` → 浏览到 `deep-learning-architectures/venv/bin/python`（Mac）或 `deep-learning-architectures\venv\Scripts\python.exe`（Windows）
