...
> 以下命令均在**仓库根目录**下开始执行，先进入模块目录 `cd 05-deep-learning-architectures`。
...
```bash
cd 05-deep-learning-architectures
...
打开 `01-cnn-basics/01-cnn-basics.ipynb`：
...
3. 选择 **Python (deep-learning-architectures)**
...
2. 仍没有则手动指定：`Python Environments...` → `Enter interpreter path...` → 浏览到 `05-deep-learning-architectures/venv/bin/python`（Mac）或 `05-deep-learning-architectures\venv\Scripts\python.exe`（Windows）
...
在网页中 **Kernel → Change Kernel** 选择 `Python (deep-learning-architectures)`。
...
jupyter kernelspec remove deep-learning-architectures
```
