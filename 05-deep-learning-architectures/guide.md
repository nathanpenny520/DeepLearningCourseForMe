...
> 以下命令均在**仓库根目录**下开始执行，先进入模块目录 `cd deep-learning-architectures`。
...
2. 仍没有则手动指定：`Python Environments...` → `Enter interpreter path...` → 浏览到 `05-deep-learning-architectures/venv/bin/python`（Mac）或 `deep-learning-architectures\venv\Scripts\python.exe`（Windows）
