...
> 以下命令均在**仓库根目录**下开始执行，先进入模块目录 `cd linear-algebra`。
...
2. 仍没有则手动指定：`Python Environments...` → `Enter interpreter path...` → 浏览到 `01-linear-algebra/venv/bin/python`（Mac）或 `linear-algebra\venv\Scripts\python.exe`（Windows）
