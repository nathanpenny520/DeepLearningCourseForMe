...
> 以下命令均在**仓库根目录**下开始执行，先进入模块目录 `cd 01-linear-algebra`。
...
打开 `01-tensor-basics/tensor-basics.ipynb`：
...
2. 仍没有则手动指定：`Python Environments...` → `Enter interpreter path...` → 浏览到 `01-linear-algebra/venv/bin/python`（Mac）或 `01-linear-algebra\venv\Scripts\python.exe`（Windows）
...
在网页中 **Kernel → Change Kernel** 选择 `Python (linear-algebra)`。
