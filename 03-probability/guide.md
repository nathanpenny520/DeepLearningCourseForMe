...
> 以下命令均在**仓库根目录**下开始执行，先进入模块目录 `cd 03-probability`。
...
2. 仍没有则手动指定：`Python Environments...` → `Enter interpreter path...` → 浏览到 `03-probability/venv/bin/python`（Mac）或 `03-probability\venv\Scripts\python.exe`（Windows）
